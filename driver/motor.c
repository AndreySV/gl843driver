			   ...
			   unsigned int exposure)
{
	...
	const int base_ydpi = 4800;

	speed = exposure * y_dpi / (base_ydpi << type);
	...
	cs4400f_build_motor_table(&move, 200, HALF_STEP);
	...
	Ks = base_ydpi << scan.type;
	Km = base_ydpi << move.type;
	Rms = Km / Ks;
	...
	send_motor_table(dev, 1, scan.a, 1020);
	send_motor_table(dev, 2, scan.a, 1020);
	send_motor_table(dev, 3, scan.a, 1020);
